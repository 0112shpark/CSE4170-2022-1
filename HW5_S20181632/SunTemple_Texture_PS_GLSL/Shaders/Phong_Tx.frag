...
uniform LIGHT u_light[NUMBER_OF_LIGHTS_SUPPORTED+3];
...
vec4 lighting_equation_textured(in vec3 P_EC, in vec3 N_EC, in vec4 base_color) {
	...
	for (int i = 0; i < NUMBER_OF_LIGHTS_SUPPORTED+3; i++) {
...
if (u_flag_blending == 0) {
		final_color = vec4(shaded_color.rgb, 1.0f);
	}
	else { 
		// Back-to-Front if u_flag_blending == 1
		// Front-to-Back if u_flag_blending == 2

		final_color = vec4(u_fragment_alpha*shaded_color.rgb, u_fragment_alpha);
	}	
#endif
if (screen_effect) {
		float x_mod, y_mod;
